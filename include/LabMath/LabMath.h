...
namespace lab {
    ...
    using namespace polymer::math; // hoist polymer::math into lab::
    ...
    struct m44f {
        ...
        LM_API void decompose(v3f& t, v3f& r, v3f& s) const;
        LM_API void recompose(const v3f& t, const v3f& r, const v3f& s);
        LM_API void orthonormalize();
    ...
    LM_API void transpose(m44f&);
    LM_API void rotateX(m44f&, float degrees);
    LM_API void rotateY(m44f&, float degrees);
    LM_API void rotateZ(m44f&, float degrees);
    LM_API void rotate(m44f&, float degrees, float x, float y, float z);
    inline void rotate(m44f& mat, float degrees, const v3f &v) { return rotate(mat, degrees, v.x, v.y, v.z); }
    LM_API void scale(m44f&, float x, float y, float z);
    inline void scale(m44f& mat, const v3f &v) { return scale(mat, v.x, v.y, v.z); }
    LM_API void translate(m44f&, float x, float y, float z);
    inline void translate(m44f& mat, const v3f &v) { return translate(mat, v.x, v.y, v.z); }
    LM_API void ortho(m44f&, float l, float r, float b, float t, float n, float f);
    LM_API void frustum(m44f&, float l, float r, float b, float t, float n, float f);
    LM_API void perspective(m44f&, float fov, float aspect, float near, float far);
    LM_API void invert(m44f&);

    LM_API void decompose(m44f, v3f& t, v3f& r, v3f& s);
    LM_API void recompose(m44f& result, const v3f& t, const v3f& r, const v3f& s);
    LM_API void orthonormalize(m44f&);


	LM_CAPI m44f m44f_identity;
    ...
    inline  v4f matrix_multiply(const m44f& a, const v4f& v) { return a * v; }
	inline m44f matrix_multiply(const m44f& a, const m44f& b) { return a * b; }

    inline m44f matrix_invert(const m44f& a) { return linalg::inverse(a); }
    inline m44f matrix_transpose(const m44f& a) { return linalg::transpose(a); }
    ...
    inline quatf quat_inputAngularVelocity(quatf init, float dt, const v3f& a)
    {
	    quatf velquat;
	    velquat.x = a.x;	// x
	    velquat.y = a.y;	// y
	    velquat.z = a.z;    // z
	    velquat.w = 0.f;    // w
        velquat = qmul(qmul(velquat, velquat), quatf(a.x, a.y, a.z, 1.0f)) * (dt * 0.5f);
        return normalize(init + velquat);
    }

#ifdef HAVE_GLM
    inline quatf quat_fromAxisAngle(const v3f& a, float rad)
    {
        return glm::angleAxis(rad, a);
    }

    inline v3f quat_rotateVector(quatf q, v3f v)
    {
    ...
    template <class T>
    inline quatf quat_fromAxisAngle(linalg::vec<T,3> v, T a)
    {
		linalg::vec<T,4> Result;
        T s = std::sin(a * T(0.5));
		Result.w = std::cos(a * T(0.5));
		Result.x = v.x * s;
		Result.y = v.y * s;
		Result.z = v.z * s;
		return Result;
    ...
    template <class T>
    inline linalg::vec<T, 3> quat_rotateVector(linalg::vec<T, 4> q, linalg::vec<T, 3> v)
    {
        // https://gamedev.stackexchange.com/questions/28395/rotating-vector3-by-a-quaternion
        linalg::vec<T,3> u {q.x, q.y, q.z};
        T s = q.w;

        return   T(2) * dot(u, v)  * u
               + (s*s - dot(u, u)) * v
               + T(2) * s * cross(u, v);
    }
